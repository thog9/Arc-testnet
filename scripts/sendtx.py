...
# Constants
NETWORK_URL = "https://rpc.testnet.arc.network"
CHAIN_ID = 5042002
...
CONFIG = {
    ...
    "MAX_CONCURRENCY": 5,
    "MAX_RETRIES": 3,
    "MINIMUM_BALANCE": 0.001  # USDC
}

# Từ vựng song ngữ
LANG = {
    'vi': {
        'title': '✨ GỬI GIAO DỊCH - ARC TESTNET ✨',
        'info': 'ℹ Thông tin',
        'found': 'Tìm thấy',
        'wallets': 'ví',
        ...
        'enter_amount': '✦ NHẬP SỐ LƯỢNG USDC',
        'amount_prompt': 'Số lượng USDC (mặc định 0.000001, tối đa 999): ',
        'amount_unit': 'USDC',
        ...
        'insufficient_balance': 'Số dư không đủ (cần ít nhất {required:.6f} USDC cho giao dịch)',
        ...
        'connect_success': '✅ Thành công: Đã kết nối mạng ARC Testnet',
    ...
    'en': {
        'title': '✨ SEND TRANSACTION - ARC TESTNET ✨',
        'info': 'ℹ Info',
        'found': 'Found',
        'wallets': 'wallets',
        ...
        'enter_amount': '✦ ENTER AMOUNT OF USDC',
        'amount_prompt': 'Amount of USDC (default 0.000001, max 999): ',
        'amount_unit': 'USDC',
        ...
        'insufficient_balance': 'Insufficient balance (need at least {required:.6f} USDC for transaction)',
        ...
        'connect_success': '✅ Success: Connected to ARC Testnet',
    ...
    for attempt in range(CONFIG['MAX_RETRIES']):
        try:
            ...
            if balance < required_balance:
                print(f"{Fore.RED}  ✖ {LANG[language]['insufficient_balance'].format(required=required_balance)}: {balance:.6f} USDC{Style.RESET_ALL}")
                return False
...
async def process_wallet(index: int, profile_num: int, private_key: str, proxy: str, w3: Web3, choice: str, tx_count: int, amount: float, addresses: list, language: str):
    ...
    if eth_balance < CONFIG['MINIMUM_BALANCE']:
        print(f"{Fore.RED}  ✖ {LANG[language]['insufficient_balance'].format(required=CONFIG['MINIMUM_BALANCE'])}: {eth_balance:.6f} USDC{Style.RESET_ALL}")
        return 0
